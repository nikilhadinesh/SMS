...
//   const [timetable, setTimetable] = useState({ day: '', periods: Array(8).fill('') });

//  // TeacherDashboard.jsx
// // 1. New State for input
// const [regInput, setRegInput] = useState('');

// // 2. Updated findStudent logic
...
//     console.log("Sending to server:", { id: student._id, marks });
//     await axios.post('http://localhost:5000/api/teacher/add-marks', { id: student._id, marks });
//     setMarks({ subject: '', internal: '', external: '' }); // Disappear after upload
//     alert("Marks Added!");
//   };

// // Timetable upload function
// const addTimetable = async () => {
//     if(!student) return alert("Search a student first!");
//     try {
//         await axios.post('http://localhost:5000/api/teacher/add-timetable', { 
//             id: student._id, 
//             timetable: { day: timetable.day, periods: timetable.periods }
//         });
//         setTimetable({ day: '', periods: Array(8).fill('') }); // Boxes empty aagidum
...
//         style={styles.input} 
//     />
//     <button onClick={findStudent} style={styles.button}>Find Student</button>
// </div>
      
//       {student && (
//         <div style={styles.card}>
//           <h3>Add Marks for {student.name}</h3>
//           <input placeholder="Subject" value={marks.subject} onChange={e => setMarks({...marks, subject: e.target.value})} style={styles.input} />
//           <input type="number" placeholder="Internal (40)" value={marks.internal} onChange={e => setMarks({...marks, internal: e.target.value})} style={styles.input} />
//           <input type="number" placeholder="External (60)" value={marks.external} onChange={e => setMarks({...marks, external: e.target.value})} style={styles.input} />
//           <button onClick={addMarks} style={styles.button}>Submit Marks</button>
//         </div>
//       )}

//       <div style={styles.card}>
//             <h3>Update Timetable (8 Periods)</h3>
//             <input placeholder="Day (e.g., Monday)" value={timetable.day} onChange={e => setTimetable({...timetable, day: e.target.value})} style={styles.input} />
//             {timetable.periods.map((p, i) => (
//               <input key={i} placeholder={`Period ${i+1}`} value={p} onChange={e => {
//                 let newP = [...timetable.periods]; newP[i] = e.target.value;
//                 setTimetable({...timetable, periods: newP});
//               }} style={styles.input} />
//             ))}
//             <button onClick={addTimetable} style={styles.button}>Update day</button>
//           </div>
//     </div>
//   );
// }
...
// const styles = {
//   container: { padding: '40px', background: '#f0f2f5', minHeight: '100vh' },
//   card: { backgroundColor: '#fff', padding: '25px', borderRadius: '15px', marginTop: '20px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' },
//   input: { display: 'block', margin: '10px 0', padding: '10px', width: '100%' },
//   button: { padding: '10px 20px', background: '#4f46e5', color: '#fff', border: 'none', cursor: 'pointer' }
// };
